...
| **`patch_libextavrcp_jni.py`** | `libextavrcp_jni.so` — R1 redirect, T1/T2-stub trampolines, the dynamically-assembled trampoline blob (extended_T2/T4/T5/T_charset/T_battery/T_continuation/T6/T8/T9), U1 (kernel auto-repeat NOP), and the LOAD #1 program-header extension. T1 advertises events `{0x01, 0x02, 0x05, 0x08, 0x09, 0x0a, 0x0b, 0x0c}` in GetCapabilities; T8 INTERIM-acks all eight events (mirroring Pixel-4-as-TG on a 1.3-profile SDP record). T5 emits CHANGED for ev=0x09 NowPlayingContent on every track edge alongside the existing TrackChanged emit — the primary metadata-refresh trigger for at least one CT in the matrix. Blob built by `_trampolines.py` via `_thumb2asm.py`. | `--avrcp` |
...
- **`Y1T :`** — Native trampoline-side. `patch_libextavrcp_jni.py` splices `__android_log_print(INFO, "Y1T", fmt, value)` into extended_T2 / T4 / T5 / T8 / T9 emit sites (inbound RegisterNotification event_ids — `T2reg ev=` for the TRACK_CHANGED-arm in extended_T2 and `T8reg ev=` for the dispatch in T8, outbound CHANGED arg values including `T5ncc ev=09` per track-edge NowPlayingContent emit, per-attribute GEA strlen, JNI-side TID source). `patch_mtkbt.py` adds one mtkbt-side log (`M5wire c39`) hooked into the AVCTP wire-frame builder for M5 TID-echo end-to-end verification. Tail with `adb logcat -s Y1T:*`; parse GEA wire-size predictions via [`../../tools/avrcp-wire-trace.py`](../../tools/avrcp-wire-trace.py).
