...
| **`patch_y1_apk.py`** | `com.innioasis.y1*.apk` — A/B/C (Artist→Album navigation), Patch E (discrete PASSTHROUGH PLAY/PAUSE/STOP routing), Patch H (`BaseActivity.dispatchKeyEvent` propagates unhandled media keys). Uses androguard + apktool. | `--music-apk` (all of A/B/C/E/H ship together) |
