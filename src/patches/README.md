...
|---|---|---|
| **`patch_mtkbt.py`** | `mtkbt` — SDP shape (V1/V2/V3/V4/V7/V8/S1), activeVersion override (V6), force-PASSTHROUGH-emit op_code dispatch (P1), best-effort AVDTP sig 0x0c → sig 0x02 alias (V5), M1 + M6 (the RegNotif response builder's INTERIM/CHANGED discriminator: M1 widens the dispatcher cmp from 1 to 0x0F so wire ctype matches the JNI's reasonCode; M6 NOPs the hardcoded CHANGED-ctype write so the response builder's CHANGED branch becomes a pure pass-through for any non-INTERIM AV/C ctype value the JNI sets in `ipc[8]`), M2 + M3 (Path A: bypass outbound-frame builder's chip-readiness list-check + chip-busy flag set for fragmented multi-frame responses like `msg=540` GetElementAttributes), M4 (Path B: same list-check bypass on the structurally-identical twin builder for short single-PDU responses like `msg=544` RegNotif INTERIM/CHANGED, so subscription-class CTs receive subscription confirmations reliably), and M5 + M7 (TID echo trampoline: a 24-byte code-cave at `0xf3680` in LOAD #1 page-padding makes Path B's `chan[+0x29]` write conditional on `packet[+8]` (M5) and then unconditionally syncs `chan+0x39` from the per-channel inbound-RX TID stash at `chan+0xba9` (M7). M5 alone misses the `msg=544` RegNotif response path whose allocator doesn't set the outbound-discriminator byte; M7 closes that gap so the wire builder reads the correct AVCTP V13 §3.3.5 / AVRCP §6.7.2 echo TID regardless of which IPC allocator originated the response — same ELF segment-extension trick used by `patch_libextavrcp_jni.py`). | `--avrcp` |
| **`patch_libextavrcp_jni.py`** | `libextavrcp_jni.so` — R1 redirect, T1/T2-stub trampolines, the dynamically-assembled trampoline blob (extended_T2/T4/T5/T_charset/T_battery/T_continuation/T6/T8/T9), U1 (kernel auto-repeat NOP), and the LOAD #1 program-header extension. T1 advertises events `{0x01, 0x02, 0x05, 0x08, 0x09, 0x0a, 0x0b, 0x0c}` in GetCapabilities; T8 INTERIM-acks all eight events (mirroring Pixel-4-as-TG on a 1.3-profile SDP record). T5/T9 emit CHANGED for ev=0x09 NowPlayingContent on every track/play edge alongside the existing TrackChanged emit — the primary metadata-refresh trigger for at least one CT in the matrix. Per-event RegNotif transaction IDs are saved at INTERIM-ack time (encoded as `TID + 1` in the subscription gate bytes `state[16]` / `state[20]`) and restored to `conn[+0x11]` before each delayed CHANGED emit, so mtkbt's stock wire path (libextavrcp `*_rsp` reads `conn[+0x11]` → `msg[5]` → mtkbt `packet[+0xa]` → `chan+0x39` → wire builder) emits the §3.3.5 strict-echo TID even when the CHANGED fires 30 s+ after the originating RegNotif on a CT that cycles AV/C transaction IDs. Blob built by `_trampolines.py` via `_thumb2asm.py` (hard-capped at 4020 bytes by the LOAD #1 padding budget; patcher asserts on overflow). | `--avrcp` |
...
- **`Y1T :`** — Native trampoline-side. `patch_libextavrcp_jni.py` splices `__android_log_print(INFO, "Y1T", fmt, value)` into T4 / T5 / T8 / T9 emit sites (`T8reg ev=` for inbound RegisterNotification dispatch, `T5emit aid=` for outbound TrackChanged CHANGED, `T5ncc` for outbound NowPlayingContent CHANGED on track edges, `T9emit pstat=` for outbound PlaybackStatus CHANGED, `T4a=` for per-attribute GEA strlen). `patch_mtkbt.py` adds mtkbt-side logs hooked into the AVCTP wire-frame builder and into the M5+M7 TID-echo cave: `M5wire c39=` (chan+0x39 at wire emit) plus `M5dbg p8/pd/ba9=` (packet[+8], packet[+0xd], chan+0xba9 at cave exit — disambiguates which value is 0 when a CT retry-storms). Tail with `adb logcat -s Y1T:*`; parse GEA wire-size predictions via [`../../tools/avrcp-wire-trace.py`](../../tools/avrcp-wire-trace.py).
