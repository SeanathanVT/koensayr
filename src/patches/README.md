...
| **`patch_libextavrcp_jni.py`** | `libextavrcp_jni.so` — R1 redirect, T1/T2-stub trampolines, the dynamically-assembled trampoline blob (extended_T2/T4/T5/T_charset/T_battery/T_continuation/T6/T8/T9), U1 (kernel auto-repeat NOP), and the LOAD #1 program-header extension. T1 advertises `{0x01, 0x02, 0x05, 0x08, 0x09, 0x0a, 0x0b, 0x0c}` in GetCapabilities; T8 NOT_IMPLEMENTs RegisterNotification for events 0x09-0x0c on the wire (AVRCP 1.3 §5.4.2 Tbl 5.28 scope). Blob built by `_trampolines.py` via `_thumb2asm.py`. | `--avrcp` |
