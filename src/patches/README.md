...
- **`Y1T :`** — Native trampoline-side. `patch_libextavrcp_jni.py` splices `__android_log_print(INFO, "Y1T", fmt, value)` into T5 NowPlayingContent CHANGED on track edges (`T5ncc`). `patch_mtkbt.py` adds mtkbt-side logs hooked into the AVCTP wire-frame builder and into the M5 TID-echo cave: `M5wire c39=` (chan+0x39 at wire emit) plus `M5dbg p8/pd/ba9=` (packet[+8], packet[+0xd], chan+0xba9 at cave exit — disambiguates which value is 0 when a CT retry-storms). Tail with `adb logcat -s Y1T:*`.
