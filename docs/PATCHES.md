...
M3 NOPs the SET (not the CHECK). After M3 the flag is never set from the `fcn.0x6df20` site, so `cbnz r3, 0x6df52` at `0x6df3a` no longer trips from that writer. Safe because mtkbt's IPC dispatcher is single-threaded and `fcn.0xae5e4`'s downstream chain (`fcn.0xae418 → fcn.0x50918 → mtk_bt_write`) is synchronous (blocking UART write) — no concurrent emits race on the per-channel state inside `fcn.0xae5e4`. The completion handler (`fcn.0x6d9b8`) still clears the flag on ACK events; harmless no-op since the flag is already 0. Paired with M9 — there's a second writer of the same flag at `fcn.0x6d9ac:0x6dda8` (inbound-CMD path) that M3 alone leaves unpatched.

**M9 — Path A busy-flag second-writer bypass: NOP inbound-callback SET** at file `0x6dda8` (1 site, 4 bytes):

| site | bytes (before → after) | mnemonic |
|---|---|---|
| `0x6dda8` | `84 f8 f2 10` → `00 bf 00 bf` | `strb.w r1, [r4, #0xf2]` → `nop; nop` |

`fcn.0x6d9ac` is mtkbt's inbound AVRCP CMD handler. When an inbound `RegisterNotification` CMD (`PDU 0x31`) arrives, the upper-layer callback (the JNI's per-event response handler) is invoked synchronously; if it returns `CType = 0x0F` (INTERIM — the canonical "I'll respond with INTERIM and want to keep this subscription open for the CHANGED later" semantic), `fcn.0x6d9ac:0x6dda8` SETs `chan+0xf2 = 1`. The SAME flag is read at the Path A outbound gate `fcn.0x6df20:0x6df3a` (`cbnz r3, 0x6df52`) — so until the L2CAP TX-complete callback (`fcn.0x6d9ac:0x6d9f0 / 0x6da10`) clears the flag, every subsequent Path A outbound frame on that channel silently drops with rc=0xb. M3 patched only the SET at `fcn.0x6df20:0x6df42` (the "i'm about to call L2CAP_SendData" SET); the writer at `fcn.0x6dda8` was unpatched, so post-INTERIM CHANGEDs continued to drop whenever the L2CAP completion event hadn't yet fired.
...
0x6dda6: movs r1, #1
0x6dda8: strb.w r1, [r4, #0xf2]  ; chan+0xf2 = 1   ← THE UNPATCHED SET (M9 NOPs)
0x6ddac: b    0x6de94
```

Both branches converge at `0x6de94`; the only side effect of `0x6dda8` is the flag write. NOPping leaves control flow intact.

**Empirical motivation (Bolt 1326 capture, 2026-05-21):** `T9ps` fired 3× on PSC state edges; `EXTADP_AVRCP: send msg success` logged 3 matching `msg=544` IPC dispatches into mtkbt. The binary `btlog.bin` decoded at AVCTP cid 0x43 contained **zero** L2CAP `len=15 CType=0x0D` PSC CHANGED frames — every PSC CHANGED was silently dropped. Same session at the same cid showed 46× PPC CHANGED (`len=18`), 3× NPC CHANGED (`len=14`), 1× TC CHANGED (`len=22`) all delivered correctly. The selectivity was timing-based, not event-specific: PPC re-registers from the CT every second, so the inbound RegNotif callback re-sets the flag and L2CAP TX-complete clears it on a tight cycle; PSC subscribes once and never re-registers from §6.7.1-strict CTs, so the post-INTERIM stale flag persists across the 18-second pause-state window until T9 fires.

Why M3 didn't catch this: M3's commit message reasoned about `fcn.0x6df42` as "the chip-busy SET" — there's another structurally-identical SET on the same flag in the inbound path that M3 didn't audit. Discovered via byte-level RE of `fcn.0x6d9ac` (4 readers and 2 writers of `chan+0xf2` mapped).
...
|---|---|---|
| `T1pdu=%02x` | T4 dispatcher entry, immediately after `ldrb PDU` and before the cmp chain | inbound non-RegNotif AV/C CMD PDU ID. Covers GetCapabilities (0x10), PApp (0x11..0x16), InformDisplayableCharacterSet (0x17), InformBatteryStatusOfCT (0x18), GetElementAttributes (0x20), GetPlayStatus (0x30), Request/AbortContinuingResponse (0x40/0x41). RegisterNotification (0x31) is covered by `T2reg`. Absence of `T1pdu=20` after a `T5id` emit means the CT didn't re-fetch metadata after the TRACK_CHANGED CHANGED — Identifier-change-detection failed CT-side. |
| `T2reg ev=%02x` | `extended_T2` immediately after the PDU=0x31 check, before `save_event_seq_id` | confirms an inbound `RegisterNotification(ev=N)` CMD reached the JNI trampoline. Pairs with the outbound emit markers below to disambiguate "CT didn't subscribe to ev=N" (no `T2reg ev=N`) from "CT subscribed but our CHANGED gate skipped" (`T2reg ev=N` present, no matching `T5id`/`T9ps`/`T9papp`). |
| `T5id=%02x` | every `track_changed_rsp` call site (T4 reactive CHANGED, extended_T2 INTERIM, T5 proactive CHANGED) immediately after `adr_w r3, &selected_track_id` | byte 7 of the Identifier payload that's about to be shipped on the wire. Always `00` post-revert (matches §5.14.1 SELECTED). Confirms the emit fired AND the wire payload is zero (a non-zero value would indicate `.text` corruption or an unintended write to the static buffer). |
