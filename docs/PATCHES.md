...
|---|---|---|
| `T8reg ev=%02x` | `_emit_t8` entry | inbound `RegisterNotification` `event_id` for the dispatch in T8 (events 0x01 / 0x03..0x0C). Counts CT subscription requests per event. The presence of `T8reg ev=09` after the `t8_check_9` arm INTERIM-acks proves the CT is subscribing to `NowPlayingContentChanged` and `state[20]` is armed for T5/T9's downstream CHANGED emit. |
...
| `T5ncc` | `t5_changed` before `reg_notievent_now_playing_content_rsp` | fires once per track edge when the CT is subscribed to ev=0x09 (state[20] armed). No-arg format. Absence after `T5emit` means the CT never subscribed to NowPlayingContent — the primary refresh trigger on at least one CT in the matrix. |
