...
Per AVRCP 1.3 §5.4.2 + ICS Table 7 row 11, GetCapabilities is **mandatory** for any TG advertising PASS THROUGH Cat 1 (which our V1 SDP does). Advertised set: `0x01` PLAYBACK_STATUS, `0x02` TRACK_CHANGED, `0x05` PLAYBACK_POS, `0x08` PLAYER_APPLICATION_SETTING_CHANGED, plus `0x09` NOW_PLAYING_CONTENT_CHANGED, `0x0a` AVAILABLE_PLAYERS_CHANGED, `0x0b` ADDRESSED_PLAYER_CHANGED, `0x0c` UIDS_CHANGED. The four 0x09..0x0c IDs come from AVRCP 1.4+; T1 advertises them and T8 INTERIM-acks each subscription, matching Pixel-4-as-TG's observed behaviour on a 1.3-profile SDP record. NowPlayingContentChanged (ev=0x09) is load-bearing: at least one CT in the test matrix uses NowPlayingContent CHANGED (not TrackChanged CHANGED) as its primary metadata-refresh trigger and falls back to a ~20 s polling cycle if that subscription is rejected. T5/T9 emit CHANGED for ev=0x09 on every track/play edge gated on state[20] armed by T8's INTERIM ack. Subscriptions for ev=0x0a/0x0b/0x0c are INTERIM-acked but never receive CHANGED (Y1 has no multi-player / UID-database semantics).

**Per-event TID echo (AVCTP V13 §3.3.5 strict echo).** Stock libextavrcp_jni.so maintains a 15-byte global `g_avrcp_req_event_database` at vaddr `0xd2b5` (`.bss`). The stock inbound CMD dispatcher calls `_Z17saveRegEventSeqIdhh(event_id, seq_id)` (sym `0x5ee5`, file offset of call `0x6d26`) for every inbound `RegisterNotification` CMD, which writes `database[event_id] = seq_id` (the inbound AVCTP TID). Stock JNI's `notificationXxxChangedNative` natives then read this table just before invoking the corresponding `*_rsp` builder; the canonical pattern (e.g. `notificationTrackChangedNative` at `0x3c06`, `notificationPlayStatusChangedNative` at `0x3cec`) is `add ip, pc ; ldrb r2, [ip, #event_id] ; strb r2, [r_avrcp+0x19] ; blx *_rsp` — writing `database[event_id] → conn[+0x11]` immediately before the response call. The `*_rsp` builder (e.g. libextavrcp.so `0x2412`) reads `conn[+0x11]` and packs it into the outbound IPC `msg[5]`, which mtkbt's stock path (`fcn.0xf0bc:0xf1a8`) writes to `chan+0x39` for the wire builder (`fcn.0xae418`).

Our T-trampolines bypass the stock `notificationXxxChangedNative` prologues (T5 replaces `notificationTrackChangedNative` at `0x3bc0`, T9 replaces `notificationPlayStatusChangedNative` at `0x3c88`) and short-circuit the stock CMD dispatcher (extended_T2 / T8 handle the RegisterNotification CMD inline). Each of these paths must replicate the `database → conn[+0x11]` write itself. A shared `restore_conn_tid` subroutine inside the trampoline blob does this once: input `r0=conn_ptr`, `r1=event_id`; PC-relative loads `&database`, reads `database[event_id]`, writes `conn[+0x11]`, returns. Each rsp call site emits a 6-byte stub (`movs r1, #event_id ; bl restore_conn_tid`) immediately after setting `r0 = avrcp_state + 8 (= conn)` and before the rsp builder blx.

This restores AVCTP V13 §3.3.5 strict-echo correctness even when the CHANGED fires asynchronously from a music-app broadcast 30+ s after the originating RegNotif — `database[event_id]` carries the latest inbound TID for *that* event_id, independent of any other CMD traffic. CTs that cycle AV/C TIDs across `0..0xf` get the per-event ID they actually subscribed with; CTs that use a single TID (or `TID=0`) are unaffected. `state[14..20]` continue to act as pure subscription gates (`0` = unsubscribed; nonzero = subscribed) — they carry no TID information.
...
1. `reg_notievent_now_playing_content_rsp` (PLT `0x330c`, event 0x09) with `r1=0`, `r2=REASON_CHANGED` (`0x0d`). Gated on `state[20]` (sub_now_playing_content, armed by T8's INTERIM ack for ev=0x09). Primary metadata-refresh trigger for at least one CT in the test matrix.
...
| 0x09 | NOW_PLAYING_CONTENT_CHANGED | `0x330c` | INTERIM ack (empty payload). Arms `state[20]` for T5/T9's per-track/play-edge CHANGED emit. |
...
- **play_status:** compare file[792] vs state[9] (`last_play_status`). On inequality, emit `reg_notievent_playback_rsp` via PLT `0x339c` with `r1=0`, `r2=REASON_CHANGED` (`0x0d`), `r3=play_status`. Gated on `state[14]` (sub_play_status). The same edge branch reads `state[20]` (sub_now_playing_content); when armed (encoded as `TID + 1`), restores the per-event TID into `conn[+0x11]` and emits `NowPlayingContentChanged` CHANGED alongside the play-status CHANGED. Update state[9].
...
|---|---|---|
| `T5tc` | `t5_track_changed` before `track_changed_rsp` | no-arg marker confirming TRACK_CHANGED CHANGED actually emitted. Absence after a track edge means `state[16]` was never armed (no inbound `RegisterNotification(ev=02)` reached extended_T2 in this session). |
| `T9ps` | `t9_play_status_changed` before `reg_notievent_playback_rsp` | no-arg marker confirming PLAYBACK_STATUS_CHANGED CHANGED actually emitted. Absence after a play/pause edge means `state[14]` was never armed. |
| `T9papp` | `t9_papp_changed` before `reg_notievent_player_appsettings_changed_rsp` | no-arg marker confirming PLAYER_APPLICATION_SETTING_CHANGED CHANGED actually emitted. Absence after a repeat/shuffle edge means `state[15]` was never armed. |
