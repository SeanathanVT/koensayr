...
3. **GetCapabilities (0x10).** Handled at `T1_extended:0xac5c` via stock JNI's pre-R1 dispatcher path; stock writes `conn[+0x11] = inbound TID` before our R1 redirect (at JNI vaddr `0x6538`) takes effect, so no trampoline write is needed for this PDU.
...
The GetEA body implements the AVRCP 1.3 §6.6.1 Table 6.26 response contract: "If NumAttributes is set to zero, all attribute information shall be returned, else attribute information for the specified attribute IDs shall be returned by the TG."
