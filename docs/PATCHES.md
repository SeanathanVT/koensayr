...
Per AVRCP 1.3 §5.4.2 + ICS Table 7 row 11, GetCapabilities is **mandatory** for any TG advertising PASS THROUGH Cat 1 (which our V1 SDP does). Advertised set: `0x01` PLAYBACK_STATUS, `0x02` TRACK_CHANGED, `0x05` PLAYBACK_POS, `0x08` PLAYER_APPLICATION_SETTING_CHANGED, plus `0x09` NOW_PLAYING_CONTENT_CHANGED, `0x0a` AVAILABLE_PLAYERS_CHANGED, `0x0b` ADDRESSED_PLAYER_CHANGED, `0x0c` UIDS_CHANGED. The four 0x09..0x0c IDs come from AVRCP 1.4+ and are advertised here — even though the SDP profile descriptor is 1.3 — to mirror Pixel-as-TG's GetCapabilities event list. The advertisement is decoupled from per-event RegisterNotification response per AV/C §6.7.1: a CT that subscribes to an advertised event_id outside the TG's actual supported set receives an AV/C NOT_IMPLEMENTED reject from T8 (rather than INTERIM) for events 0x09-0x0c, prompting the CT to drop those event_ids from its retry set per AV/C §6.7.1.
...
| 0x09 | NOW_PLAYING_CONTENT_CHANGED | `0x330c` | reject — r2=`0x08` NOT_IMPLEMENTED |
| 0x0a | AVAILABLE_PLAYERS_CHANGED | `0x3324` | reject — r2=`0x08` NOT_IMPLEMENTED |
| 0x0b | ADDRESSED_PLAYER_CHANGED | `0x3330` | reject — r2=`0x08` NOT_IMPLEMENTED (PlayerID/UidCounter zeroed, spec-irrelevant on reject) |
| 0x0c | UIDS_CHANGED | `0x3318` | reject — r2=`0x08` NOT_IMPLEMENTED (UidCounter zeroed) |

Events 0x01-0x08 cover AVRCP 1.3 §5.4.2 (Tbls 5.29/5.31/5.32/5.33/5.34/5.36/5.37) and are handled with INTERIM acks. Events 0x09-0x0c are AVRCP 1.4+ event IDs whose response builders are already linked by `libextavrcp_jni.so` (their PLT stubs are present though stock JNI never invokes them); T8 calls each helper with `r2 = 0x08` so mtkbt's M1 + M6 dispatch at `fcn.0x121d8` emits AV/C ctype `0x08` NOT_IMPLEMENTED on the wire — the spec-correct §6.7.1 response for an event_id outside the TG's supported set (T1's GetCapabilities advertisement of 0x09-0x0c is decoupled from per-event RegisterNotification response per AV/C §6.7.1). On receiving NOT_IMPLEMENTED, a spec-conforming CT drops the event_id from its retry set and does not re-register it for the rest of the session.
