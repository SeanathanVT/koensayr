...
|---|---|---|
| **V1, V2, V3, V4, V5, V6, V7, V8, S1, P_PN0, P_PN1, P1, M1, M2, M3, M4, M5, M6, M8** | `mtkbt` | SDP shape (AVRCP 1.0→1.3, AVCTP 1.0→1.2, A2DP/AVDTP 1.0→1.3, sig 0x0c→0x02 alias, internal `activeVersion` 10→14 to route the dispatcher to the AVRCP 1.3 served record, drop AdditionalProtocolDescriptorList Browse-PSM advertisement (AVRCP 1.4 §8 Table 8.2 introduced; absent from AVRCP 1.3 §6 Table 6.2), clear stock GroupNavigation feature bit (Y1 doesn't implement the Group Navigation PASSTHROUGH PDUs), ServiceName-for-SupportedFeatures swap, BrowseGroupList-for-ProviderName swap (matches the reference AVRCP 1.3 TG SDP shape's `0x0102` presence), force-PASSTHROUGH-emit op_code dispatch, RegNotif INTERIM/CHANGED dispatch cmp constant widened from 1 to 0x0F so wire ctype matches the JNI trampoline's reasonCode plus a paired NOP of the hardcoded CHANGED-ctype write so the response builder's CHANGED branch becomes a pure pass-through for any non-0x0F AV/C ctype, three NOPs across the two outbound-frame builders that remove the chip-readiness list-contains check on each path + the chip-busy flag SET on the multi-frame path so the matching CHECK never trips, a code-cave trampoline in Path B that conditional-stores `chan[+0x29]` so outbound responses echo the inbound AV/C transId per AVCTP V13 §6.5 / §6.7.2 instead of clobbering to 0, and a NOP of the `AVRCP_HandleA2DPInfo` info=1 disconnect call so the AVCTP control channel survives AVDTP CLOSE/REOPEN cycles per AVRCP V13 §4 transport independence). |
...
The mtkbt daemon ships two physical AVRCP TG SDP record templates in `.data.rel.ro`. The internal `activeVersion` field selects which is served on the wire: stock = 10 (legacy 1.0 record), V6 → 14 (AVRCP 1.3 record). V1/V2/S1 patch the legacy record (kept for the fall-through path); V7/V8/P_PN0/P_PN1 patch the AVRCP 1.3 record (where V6 routes the daemon by default) so it conforms to AVRCP 1.3 §6 Table 6.2 SDP record shape — no AdditionalProtocolDescriptorList (a 1.4-introduced attribute per AVRCP 1.4 §8 Table 8.2), Group Navigation feature bit cleared (the bit exists in 1.3 §6 Table 6.2 but Y1 doesn't implement the Group Navigation PASSTHROUGH PDUs), and `0x0102 ProviderName` present for parity with a reference AVRCP 1.3 TG's SDP shape.
...
**P_PN0 — write SDP TEXT_STR_8 " " at `0x0eb938` (4 bytes)** + **P_PN1 — `0x0005 BrowseGroupList` → `0x0102 ProviderName` at `0x0f978c` (12 bytes)**:

| | bytes | shape |
|---|---|---|
| P_PN0 before | `00 00 00 00` | (unused padding inside the SDP data area) |
| P_PN0 after  | `25 02 20 00` | SDP TEXT_STR_8 ds + len=2 + `' '` + `'\0'` |
| P_PN1 before | `05 00 05 00 3d ba 0e 00 00 00 00 00` | attr=`0x0005`, len=5, ptr=`0x0eba3d` (→ SEQ UUID16 `0x1002` PublicBrowseRoot) |
| P_PN1 after  | `02 01 04 00 38 b9 0e 00 00 00 00 00` | attr=`0x0102`, len=4, ptr=`0x0eb938` (→ TEXT_STR_8 `" "`) |

Reshapes the AVRCP 1.3 TG served record to match the Pixel-as-TG wire shape exactly. Pixel ships attribute `0x0102 ProviderName` with a single-space value alongside `0x0100 ServiceName`; Y1 stock had only `BrowseGroupList=PublicBrowseRoot` in this slot. Per Bluetooth Core SDP, `0x0005 BrowseGroupList` is OPTIONAL: absence drops the record from public browse-group searches, but spec-conforming CTs (including every device in the test matrix) discover services via UUID search against attribute `0x0001 ServiceClassIDList` which the TG record still ships. `P_PN0` writes the SDP descriptor into a 36-byte zero-padded region between two unrelated SDP data blocks; `P_PN1` reuses the now-freed entry slot. Goal: closer parity with a reference AVRCP 1.3 TG's SDP shape — `0x0102` presence is one of the few remaining wire-level deltas.
...
Wire trigger (dual-bolt-20260520-1543 t=603547):
1. CT sends AVDTP `CLOSE` (sig 0x08, ACP_SEID=1) on cid 0x42 — normal stream teardown per AVDTP V13 §8.13 (STREAMING → OPEN → IDLE state transition).
...
Per-event TID echo correctness depends on the JNI side restoring `conn[+0x11] = g_avrcp_req_event_database[event_id]` immediately before every `*_rsp` builder call (see `patch_libextavrcp_jni.py` "Per-event TID echo" section below). With that restore in place, `fcn.0xf0bc:0xf1a8` writes the per-event TID to chan+0x39, and this cave preserves it across the outbound strb at `0x6d186`.
...
T1 also calls the shared `clear_event_database` subroutine on every GetCapabilities to zero `g_avrcp_req_event_database` (15 bytes at vaddr `0xd2b5`) and `g_y1_avrcp_track_identifier` (8 bytes at `0xd2c4`). GetCapabilities is the canonical CT-connection-boundary marker (every spec-conforming CT issues it as the first PDU on a new AVCTP channel), so this gives clean session-scope semantics: stale per-event subscription bytes from a previous CT's connection cannot leak into a fresh session.

**Per-event TID echo + subscription gating (AVCTP V13 §3.3.5 strict echo).** libextavrcp_jni.so maintains a 15-byte global `g_avrcp_req_event_database` at vaddr `0xd2b5` (`.bss`) — one byte per event_id. Stock JNI's inbound CMD dispatcher path writes this table only via the canonical `notificationXxxChangedNative` natives (e.g. `notificationTrackChangedNative` at `0x3c06`, `notificationPlayStatusChangedNative` at `0x3cec`) whose pattern is `add ip, pc ; ldrb r2, [ip, #event_id] ; strb r2, [r_avrcp+0x19] ; blx *_rsp` — reading `database[event_id]` into `conn[+0x11]` immediately before the response call. The `*_rsp` builder (e.g. libextavrcp.so `0x2412`) reads `conn[+0x11]` and packs it into the outbound IPC `msg[5]`, which mtkbt's stock path (`fcn.0xf0bc:0xf1a8`) writes to `chan+0x39` for the wire builder (`fcn.0xae418`).

Our T-trampolines short-circuit the stock CMD dispatcher and bypass the canonical natives (extended_T2 / T8 handle inbound `RegisterNotification` CMDs; T5 / T9 emit outbound CHANGED on music-app broadcasts), so they take over both ends of the table. A shared `save_event_seq_id` subroutine writes `database[event_id] = seq_id + 1` on every inbound CMD reaching extended_T2 (the `+1` lets the byte unambiguously encode "not subscribed this session" as `0` even when the wire `seq_id` happens to be 0). A shared `restore_conn_tid` subroutine, called at every outbound CHANGED emit site, PC-relative loads `&database`, reads `database[event_id]`, and — if nonzero — writes `(value - 1) → conn[+0x11]`.

A nonzero byte therefore means *both* "the CT subscribed this event in the current session" *and* "this is the TID to echo." T5 / T9 use the same byte as a subscription gate, suppressing the CHANGED emit entirely when `database[event_id] == 0`. This restores AVCTP V13 §3.3.5 strict-echo correctness even when CHANGED fires 30+ s after the originating RegNotif — `database[event_id]` carries the latest inbound TID for *that* event_id, independent of other CMD traffic.

Session scope is enforced by `clear_event_database` at every T1 GetCapabilities entry — zeroing the 15-byte database + 8-byte track-identifier counter in one 6-store sequence — so the gates and Identifier counter are bound to the CT-connection boundary, not to process lifetime.
...
2. Calls `incr_and_get_track_identifier` to bump the per-session 8-byte monotonic counter at `g_y1_avrcp_track_identifier` (vaddr `0xd2c4`, `.bss` padding adjacent to the event database) — byte 7 increments by 1 per emit.
3. Calls `restore_conn_tid` to write `database[2] - 1 → conn[+0x11]` so the response builder echoes the inbound TID.
4. Replies INTERIM via `reg_notievent_track_changed_rsp` (PLT `0x3384`) with `r1=0` (success), `r2=REASON_INTERIM` (`0x0f`), `r3=&g_y1_avrcp_track_identifier` (8-byte counter).
...
**Identifier payload** = 8-byte monotonic counter (`g_y1_avrcp_track_identifier` in `.bss`) — bytes 0..6 stay zero, byte 7 increments by 1 on every TRACK_CHANGED emit (INTERIM via extended_T2, reactive CHANGED via T4, proactive CHANGED via T5). AVRCP 1.3 §6.7.2 specifies that the Identifier "shall be set to 0x00…00" for 1.3 TGs that don't support Browseable Player UIDs. Empirically at least one CT in the test matrix gates metadata refetch on Identifier-change detection across successive TRACK_CHANGED frames and never refetches when the same Identifier repeats — matching the wire shape of a reference 1.3 TG (Pixel-4-as-TG ships a monotonic 8-byte counter rather than zero). The monotonic value is treated by spec-compliant 1.3 parsers as opaque, and a reference TG that ships the same shape settles the spec-vs-empirical tension in favour of "ship what works."
...
4. `reg_notievent_track_changed_rsp` (PLT `0x3384`, event 0x02 — Tbl 5.30) with `r1=0`, `r2=REASON_CHANGED`, `r3=&g_y1_avrcp_track_identifier` (post-`incr_and_get_track_identifier` — byte 7 is now `(prev + 1) & 0xff`). Gated on `database[2] != 0`.
...
`g_avrcp_req_event_database` (15 bytes at vaddr `0xd2b5`) and `g_y1_avrcp_track_identifier` (8 bytes at vaddr `0xd2c4`) live in pre-existing `.bss` padding — they cost zero bytes in LOAD #1's budget because `.bss` is zero-filled at load time and our `clear_event_database` subroutine re-zeros them on each CT-connection boundary anyway.
...
The music app is the canonical writer of the 2213-byte double-buffer `y1-track-info` schema (1-byte `active_slot` + 3 B pad + 2 × 1104-byte slots + 1 B pad) and the 2-byte `y1-papp-set` (initial create). Both files live in `/data/data/com.innioasis.y1/files/`. The trampoline chain in `libextavrcp_jni.so` reads `y1-track-info` via `mmap2` + active-slot dispatch (see [`patch_libextavrcp_jni.py`](#patch_libextavrcp_jnipy) preamble), accesses trampoline edge state via PC-relative loads from `.bss` at `G_Y1_TRAMPOLINE_STATE_VADDR = 0xd2a4` (zero syscalls), and writes `y1-papp-set` via `open + write + close` (rare — only on CT-initiated PApp Set).
...
| Trampoline-state byte dump | `TrackInfoWriter.wakeTrackChanged` → `wTC.pre` + `wTC.post` (post via `Handler.postDelayed(50 ms)`). Same for `wakePlayStateChanged` → `wPSC.pre` + `wPSC.post`. Format: `tramp.state[0..19]=HH …` × 20 hex bytes. Authoritative semantics from `src/patches/_trampolines.py` (T9 stack frame): `[0..7]`=last-synced track_id mirror, `[9]`=last_play_status, `[10]`=last_battery_status, `[11]`=last_repeat_avrcp, `[12]`=last_shuffle_avrcp, `[13..23]`=padding / legacy bytes (the trampoline no longer reads or writes these — per-event subscription state moved into the JNI's `g_avrcp_req_event_database` global at vaddr `0xd2b5`, `.bss`, session-scope). Successive pre/post captures answer whether T5/T9 actually emitted CHANGED on the wire via the `last_*` mirror bytes at `[9..12]` (e.g., `[9]` flips from `1` to `2` after a PAUSE if T9 emitted PLAYBACK_STATUS_CHANGED). |
