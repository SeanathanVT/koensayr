...
Per AVRCP 1.3 §5.4.2 + ICS Table 7 row 11, GetCapabilities is **mandatory** for any TG advertising PASS THROUGH Cat 1 (which our V1 SDP does). Advertised set: `0x01` PLAYBACK_STATUS, `0x02` TRACK_CHANGED, `0x05` PLAYBACK_POS, `0x08` PLAYER_APPLICATION_SETTING_CHANGED, plus `0x09` NOW_PLAYING_CONTENT_CHANGED, `0x0a` AVAILABLE_PLAYERS_CHANGED, `0x0b` ADDRESSED_PLAYER_CHANGED, `0x0c` UIDS_CHANGED. The four 0x09..0x0c IDs come from AVRCP 1.4+ and are advertised here — even though the SDP profile descriptor is 1.3 — to mirror Pixel-as-TG's GetCapabilities event list. T8 NOT_IMPLEMENTs RegisterNotification for events 0x09-0x0c on the wire (AVRCP 1.3 §5.4.2 Tbl 5.28 defines events 0x01-0x08 only; the CapabilityID 0x03 advertisement is decoupled from the per-event RegisterNotification response per AV/C §6.7.1).
...
1. `reg_notievent_now_playing_content_rsp` (PLT `0x330c`, event 0x09) with `r1=0`, `r2=REASON_CHANGED` (`0x0d`). Gated on `state[20]` (sub_now_playing_content); T8 NOT_IMPLEMENTs RegisterNotification for 0x09 so the gate never arms and this branch is dead — kept for code structure only.
...
Events 0x01-0x08 cover AVRCP 1.3 §5.4.2 (Tbls 5.29/5.31/5.32/5.33/5.34/5.36/5.37). Events 0x09-0x0c (NOW_PLAYING_CONTENT_CHANGED / AVAILABLE_PLAYERS_CHANGED / ADDRESSED_PLAYER_CHANGED / UIDS_CHANGED) are AVRCP 1.4+ event IDs outside the §5.4.2 Tbl 5.28 set; T8 falls through to the `t8_unknown_event` path and emits an AV/C NOT_IMPLEMENTED reject. T1's GetCapabilities advertisement retains 0x09-0x0c for SDP-vs-wire continuity with Pixel-as-TG; the CapabilityID 0x03 advertisement is decoupled from the per-event RegisterNotification response per AV/C §6.7.1.
...
- **play_status:** compare file[792] vs state[9] (`last_play_status`). On inequality, emit `reg_notievent_playback_rsp` via PLT `0x339c` with `r1=0`, `r2=REASON_CHANGED` (`0x0d`), `r3=play_status`. Gated on `state[14]` (sub_play_status). In the same edge branch, also reads `state[20]` (sub_now_playing_content); branch never taken because T8 NOT_IMPLEMENTs RegisterNotification for event 0x09 — kept for code structure only. Update state[9].
