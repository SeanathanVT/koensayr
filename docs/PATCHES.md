...
**Subscription byte encoding (ev=0x02 and ev=0x09).** `state[16]` (sub_track_changed) and `state[20]` (sub_now_playing_content) carry the originating `RegisterNotification` CMD's AVCTP transaction ID alongside the subscription state. Encoding: `0` = not subscribed, `1..16` = subscribed with `TID = byte − 1`. The +1 offset makes `state[N] = 0` unambiguously mean "not subscribed" even when the CT subscribed with `TID = 0`. The existing gate-check idiom (`ldrb r0, state[N]; cmp r0, 0; beq skip_emit`) still works correctly — nonzero means subscribed. At INTERIM-emit time (extended_T2 ev=0x02, T8 ev=0x09) the trampoline computes `state[N] = conn[+0x11] + 1` (`conn[+0x11]` holds the inbound CMD's TID, stable across the JNI response-builder call). At CHANGED-emit time (T5 NowPlayingContent, T5 TrackChanged, T9 NowPlayingContent) the trampoline reads `state[N]`, subtracts 1, and writes the result to `conn[+0x11]` *before* calling the response builder. mtkbt's stock outbound path (`libextavrcp.so::*_rsp reads conn[+0x11] → msg[5]` → `mtkbt fcn.0x11894:0x11928 packet[+0xa] = msg[5]` → `fcn.0xf0bc:0xf1a8 chan+0x39 = packet[+0xa]` → wire builder `fcn.0xae418:0xae448 ldrb r6, [r4, 0x15]`) then carries the per-event TID through to the wire AVCTP byte 0 (TID nibble), restoring AVCTP V13 §3.3.5 strict-echo correctness even when the CHANGED fires asynchronously from a music-app broadcast 30+ s after the originating RegNotif. Without this encoding, `conn[+0x11]` reflects whichever inbound CMD's TID arrived most recently (CTs that cycle AV/C TIDs across 0..0xf rotate this value rapidly), so the CHANGED wire frame echoes the wrong TID and the CT's AVCTP layer silently rejects it.
