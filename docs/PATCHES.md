...
| `T5id=%02x` | every `track_changed_rsp` call site (T4 reactive CHANGED, extended_T2 INTERIM, T5 proactive CHANGED) immediately after `incr_and_get_track_identifier` returns | post-increment counter byte at `g_y1_avrcp_track_identifier[7]`. Confirms TRACK_CHANGED rsp actually emitted AND identifies the Identifier value shipped on the wire. Successive emits must show strictly-increasing values (mod 256) — a repeated value across emits means `incr_and_get_track_identifier` isn't running or `.bss` got clobbered, and CTs that gate metadata refetch on Identifier change won't refresh. |
