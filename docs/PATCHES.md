...
Per AVRCP 1.3 §5.4.2 + ICS Table 7 row 11, GetCapabilities is **mandatory** for any TG advertising PASS THROUGH Cat 1 (which our V1 SDP does). Advertised set: `0x01` PLAYBACK_STATUS, `0x02` TRACK_CHANGED, `0x05` PLAYBACK_POS, `0x08` PLAYER_APPLICATION_SETTING_CHANGED, plus `0x09` NOW_PLAYING_CONTENT_CHANGED, `0x0a` AVAILABLE_PLAYERS_CHANGED, `0x0b` ADDRESSED_PLAYER_CHANGED, `0x0c` UIDS_CHANGED. The four 0x09..0x0c IDs come from AVRCP 1.4+; T1 advertises them and T8 INTERIM-acks each subscription, matching Pixel-4-as-TG's observed behaviour on a 1.3-profile SDP record. NowPlayingContentChanged (ev=0x09) is load-bearing: at least one CT in the test matrix uses NowPlayingContent CHANGED (not TrackChanged CHANGED) as its primary metadata-refresh trigger, and falls back to a ~20 s polling cycle if that subscription is rejected. T5 emits CHANGED for ev=0x09 on every track edge, alongside the existing TrackChanged emit. Subscriptions for ev=0x0a/0x0b/0x0c are INTERIM-acked but never receive CHANGED (Y1 has no multi-player / UID-database semantics — the events stay idle in steady state, matching Pixel-TG's observed behaviour).
...
| 0x09 | NOW_PLAYING_CONTENT_CHANGED | `0x330c` | INTERIM ack (empty payload). Arms `state[20]` for T5's per-track-edge CHANGED emit. |
...
- **play_status:** compare file[792] vs state[9] (`last_play_status`). On inequality, emit `reg_notievent_playback_rsp` via PLT `0x339c` with `r1=0`, `r2=REASON_CHANGED` (`0x0d`), `r3=play_status`. Gated on `state[14]` (sub_play_status). Update state[9].
...
| `T5ncc ev=%02x` | `t5_changed` before `reg_notievent_now_playing_content_rsp` | fires once per track edge when the CT is subscribed to ev=0x09 (state[20] armed by T8's INTERIM ack). Constant `09`. Absence after T5emit means the CT never subscribed to NowPlayingContent — the primary refresh trigger on at least one CT in the matrix. |
