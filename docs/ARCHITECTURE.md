...
| P_PN0 | mtkbt 0x0eb938 | Write SDP TEXT_STR_8 `" "` (4 bytes: `25 02 20 00`) into a zero-padded gap in the SDP data area. Consumed by P_PN1's new entry pointer. |
| P_PN1 | mtkbt 0x0f978c | Replace AVRCP 1.3 TG record's 0x0005 BrowseGroupList slot with 0x0102 ProviderName pointing at P_PN0's descriptor. Matches a reference AVRCP 1.3 TG's SDP shape (Pixel-as-TG observed). |
