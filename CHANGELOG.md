...
- Spec-correct rejection of AVRCP 1.4+ notification events on a 1.3 target. Y1 advertises AVRCP 1.3 but, for head-unit-compatibility reasons (mirrors a known-working AVRCP 1.3 reference target), the GetCapabilities response still lists the four AVRCP 1.4+ event IDs (NowPlayingContent / AvailablePlayers / AddressedPlayer / UIDs). Y1 now rejects RegisterNotification commands for those four events with an AV/C NOT_IMPLEMENTED response on the wire, per AVRCP 1.3 §6.7.1, so a head unit that speculatively subscribes drops the events from its retry set rather than waiting indefinitely for notifications Y1 has no semantic source for. Head units that cycle through subscriptions sequentially no longer stall on those events.
