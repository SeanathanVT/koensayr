...
## [Unreleased]
### Fixed
- Interrupt-driven metadata refresh on strict AVRCP 1.3 head units. Y1 now emits a small monotonic counter as the `Identifier` field in every `TRACK_CHANGED` notification — starting at zero per session and incrementing on every track edge — matching the wire shape Pixel-4-as-TG ships in AVRCP-1.3 mode. Previously, Y1 emitted the internal per-track audio_id (an AVRCP 1.4+ Browseable-Player-UID-style value that strict 1.3 parsers silently dropped). A subsequent change to the strictest 1.3 reading (eight zero bytes per AVRCP 1.3 §6.7.2: "the Identifier shall always be set to 0x00...00") fixed the strict-1.3 parsers but introduced a new failure mode on head units that gate metadata refetch on Identifier-change-detection — those units saw the same eight-zero Identifier on every emit and skipped the refetch, leaving metadata frozen after the first track. The Pixel-style monotonic counter is the same wire shape Pixel ships and is empirically the only one that works across both strict-1.3 parsers and change-detection-gated head units. The counter resets on every fresh head-unit GetCapabilities, which is the AVRCP 1.3 §5.4.1 setup-flow signal for a new connection.
- Interrupt-driven metadata refresh on head units that gate refresh on `NowPlayingContentChanged`. Y1 now ACKs all four AVRCP 1.4+ event subscriptions (`NowPlayingContent`, `AvailablePlayers`, `AddressedPlayer`, `UIDs`) with `INTERIM` rather than `NOT_IMPLEMENTED`, mirroring the working Pixel-4-as-TG reference. Y1 also emits a fresh `NowPlayingContentChanged` notification on every track edge alongside the existing `TrackChanged` notification. Some head units treat `NowPlayingContentChanged` (not `TrackChanged`) as the primary signal to re-fetch metadata; without it they fall back to a ~20 s polling cycle, producing a long visible lag between the song actually changing and the head unit's display updating.
...
- AVRCP transaction-label echoing on head units that rotate AVCTP transaction IDs across the 0-15 range — now correct for asynchronous CHANGED notifications fired on track-edge / play-state-edge broadcasts. Head units that pair each `RegisterNotification` command with a unique transaction ID expect the matching `INTERIM` *and* `CHANGED` response to carry that same ID. The transaction ID is now sourced per event from the stock per-event saved-seq-id table at every response site, so head units that gate metadata refresh on subscription confirmations correctly receive interrupt-driven updates instead of falling back to slow polling. Head units that use a single transaction ID (or transaction ID 0) for all commands are unaffected by this change.
...
- Spurious paused-state notifications during track changes. The music app's track-switch pause→play handshake briefly transitioned through the paused state before the new track started playing; that paused state was reaching the head unit as a `PLAYBACK_STATUS_CHANGED` notification within ~300 ms of the matching playing notification, tripping some head units' rapid-state-change back-off and causing them to stop refreshing their playback indicator. The transient is now suppressed; head units see only the final `playing` notification for the new track.
- Continuous metadata + playback-state refresh on head units that don't re-`RegisterNotification` between every CHANGED. Y1 previously gated each event's CHANGED notifications behind a single subscription token that was consumed by the first CHANGED — head units that subscribed once and trusted Y1 to keep emitting on every value change would see the first CHANGED (track-1 metadata, first play/pause toggle, first repeat/shuffle UI sync, the start of the playhead) then nothing thereafter, leaving the pane frozen on the initial state. Each per-event subscription is now treated as session-long; CHANGED fires on every value edge for the lifetime of the subscription.
- No more ghost subscriptions across head-unit reconnects. Y1 previously persisted per-event subscription state on disk to survive reboots, which meant a head unit that subscribed for play-status notifications in one Bluetooth session would still appear "subscribed" the next time Y1 booted — even if a different head unit was now connected. Strict head units that received those unsolicited notifications would reject the AVCTP traffic and stop using AVRCP entirely for the rest of the session. Y1 now scopes each subscription to the current Bluetooth-stack process lifetime: subscriptions are forgotten when the Bluetooth process restarts (including across Y1 reboots), so each fresh connection only receives notifications for events the currently-connected head unit explicitly subscribes to in this session.
