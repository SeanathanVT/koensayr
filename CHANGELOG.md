...
## [Unreleased]
### Fixed
- Bluetooth metadata + playback indicators now work on head units that cycle AVCTP transaction IDs (a common pattern in strict-§3.3.5 implementations). Every AVRCP response Y1 emits — both event notifications and direct query replies (GetElementAttributes, GetPlayStatus, character set / battery acks, PlayerApplicationSetting PDUs) — now echoes the inbound command's transaction ID back to the head unit. Without this echo, strict head units rejected the responses per AVCTP §3.3.5 and silently fell back to forwarding only key presses; metadata panes stayed blank and play-state indicators desynchronised.
