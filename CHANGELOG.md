...
## [Unreleased]
### Fixed
- Track-metadata refresh on head units that cycle their AVRCP event subscriptions sequentially. Stock trampoline-chain INTERIM-acked four AVRCP 1.4+ notification events (NowPlayingContent / AvailablePlayers / AddressedPlayer / UIDs) so a head unit subscribing to them would never receive a CHANGED response. On a head unit that polls events sequentially rather than maintaining all subscriptions in parallel, its cursor parked on one of those events and stalled — once-stuck, the head unit stopped re-subscribing to the events Y1 actually drives (TrackChanged / PlaybackStatusChanged), so the metadata pane froze on the track that was playing at the time of the stall. Y1 now rejects those four 1.4+ events with NOT_IMPLEMENTED on the wire (their advertisement in GetCapabilities is retained for SDP-vs-wire continuity with Pixel-as-TG, per AV/C §6.7.1's decoupling of advertisement from per-event response).
