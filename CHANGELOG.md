...
- AVRCP transaction-label echoing on head units that rotate AVCTP transaction IDs across the 0-15 range — now correct for subscription confirmations as well. The earlier fix preserved the transaction ID on most response types but missed the specific path used for `RegisterNotification` acknowledgements, so head units that gate their metadata refresh on those acks would silently abandon the connection or fall back to a slow polling refresh after a few retries. Every outbound AVRCP response now carries the matching transaction ID, restoring interrupt-driven metadata refresh on these head units.
