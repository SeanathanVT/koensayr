...
## [Unreleased]
### Fixed
- Interrupt-driven metadata refresh on strict AVRCP 1.3 head units. Y1 now emits the spec-mandated 8 zero bytes as the `Identifier` field in every `TRACK_CHANGED` notification (per AVRCP 1.3 §6.7.2: "the Identifier shall always be set to 0x00...00"). Previously, Y1 emitted the internal per-track audio_id — a value valid only under the AVRCP 1.4+ Browseable Player extension — which strict 1.3 parsers silently dropped, leaving them to refresh metadata only via their fallback polling cycle (~22 s lag between track edge and next metadata fetch). On permissive head units the change is byte-equivalent for refresh behaviour. The trampoline still tracks the per-track audio_id internally for edge detection; only the wire payload is constrained to spec.
